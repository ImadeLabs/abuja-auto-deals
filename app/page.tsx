...
import { 
  ...
  PlayCircle, Camera, X, ChevronLeft, Tag, MapPin, MessageSquare
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans selection:bg-blue-100">
      ...
      <AnimatePresence>
        {selectedImg !== null && (
          ...
          >
            <button className="absolute top-6 right-6 text-white p-2 hover:bg-white/10 rounded-full transition-colors z-[110]">
              <X size={32} />
            </button>
            <button onClick={prevImg} className="absolute left-4 text-white p-4 hover:bg-white/10 rounded-full transition-colors z-[110]">
              <ChevronLeft size={40} />
            </button>
            <motion.img 
              key={selectedImg}
            ...
            />
            <button onClick={nextImg} className="absolute right-4 text-white p-4 hover:bg-white/10 rounded-full transition-colors z-[110]">
      ...
      {/* Status Header */}
      <div className="bg-[#1E40AF] text-white py-2.5 px-4 text-center text-[10px] md:text-xs font-black tracking-widest uppercase sticky top-0 z-50 shadow-md">
        <span className="flex items-center justify-center gap-2">
          <Ship size={14} className="animate-pulse" /> USA DIRECT IMPORT | NATIONWIDE DELIVERY
        </span>
      </div>
      ...
      <header className="bg-white border-b border-slate-200 py-12 px-6">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-5xl md:text-7xl font-black mb-4 tracking-tighter leading-none text-slate-900 uppercase">
              TOYOTA <br/><span className="text-blue-600 tracking-tighter">Camry 2013</span>
            </h1>
            ...
            {/* Hero Action Button */}
            <a href={whatsappLink} className="bg-blue-600 text-white px-8 py-5 rounded-2xl font-black text-center flex items-center justify-center gap-3 hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/20 active:scale-95 group tracking-wide">
              <MessageSquare size={20} className="group-hover:animate-bounce" /> DM FOR INFO
            </a>
          </div>

          <div className="relative">
            <div className="aspect-video bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl relative border-4 border-white ring-1 ring-slate-100">
               <video 
                 autoPlay 
                 muted 
                 loop 
                 ...
                 controls 
                 preload="auto"
                 poster="/camry2013se (1).jpg"
                 className="w-full h-full object-cover"
               >
                 <source src="/camry2013se.mp4" type="video/mp4" />
                 Your browser does not support the video tag.
               </video>
               <div className="absolute top-4 left-4 bg-black/40 backdrop-blur-md px-3 py-1 rounded-full text-[9px] font-bold text-white uppercase tracking-widest pointer-events-none">
                 Live Walkthrough
               </div>
            </div>
          </div>
      ...
      <section className="py-12 px-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {images.map((img, index) => (
            ...
            >
              <img src={img} className="w-full h-full object-cover" alt="Camry Detail" />
        ...
        <div className="bg-slate-900 p-10 rounded-[3rem] text-white flex flex-col md:flex-row items-center justify-between gap-8 border border-slate-800 shadow-2xl">
          <div>
            <h2 className="text-3xl font-black mb-2 tracking-tight">Price: {price}</h2>
            <p className="text-blue-300 font-medium tracking-wide flex items-center gap-2">
              <CheckCircle2 size={16} /> Original Duty Fully Paid
            </p>
          </div>
          <a href={whatsappLink} className="bg-blue-600 hover:bg-white hover:text-blue-950 px-12 py-5 rounded-full font-black transition-all flex items-center gap-2 shadow-lg tracking-wide uppercase">
            DM FOR INFO <ChevronRight size={20} />
          </a>
        </div>
      </section>
